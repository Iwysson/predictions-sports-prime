...
function defineLeague(
  config: Omit<LeagueConfig, "season" | "display" | "sources"> & { sourcePath: string }
): LeagueConfig {
  const { sourcePath, ...metadata } = config;
  const feed = source(sourcePath);
  return {
    ...metadata,
    season: "2026/27",
    display: { showOnHome: config.featured, showStandings: true },
