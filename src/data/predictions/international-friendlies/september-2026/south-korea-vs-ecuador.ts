import type { EditorialPrediction } from "@/types";

const publishedAt = "2026-09-21T12:12:53-03:00";
const updatedAt = "2026-09-23T06:50:58-03:00";

export const southKoreaVsEcuador: EditorialPrediction = {
  league: "international-friendlies",
  homeTeam: "South Korea",
  awayTeam: "Ecuador",
  slug: "south-korea-vs-ecuador",
  title: "South Korea vs Ecuador Prediction, Odds and Betting Tips",
  analysis: [`# South Korea vs Ecuador Prediction, Odds and Betting Tips

**Prediction:** South Korea double chance (1X) + Over 1.5 goals  
**Odds:** 1.98

### Match information

**International Friendly — 24 September 2026 — Suwon World Cup Stadium, South Korea.** The fixture opens Robert Moreno's spell as South Korea's interim head coach and is also Marcelo Gallardo's first match in charge of Ecuador, giving both sides a genuine tactical reset immediately after the World Cup cycle.

| Prediction | Odds | Estimated probability |
|---|---:|---:|
| **South Korea double chance (1X) + Over 1.5 goals** | **1.98** | **51%** |

### Team news, call-ups and projected lineups

Moreno named a **30-player South Korea squad** for the four-match run against Ecuador, Uruguay, Venezuela and Uzbekistan. The established core still contains **Son Heung-min, Lee Kang-in, Kim Min-jae and Lee Jae-sung**, while Rangers winger Kim Min-su received his first senior call-up and Jeong Seung-won was another notable selection. Twelve members of the squad are based in the K League, giving Moreno a mix of domestic players and the established European-based core.

Gallardo announced his first **26-player Ecuador squad** on 20 September. The most significant team-news points are the absences of **Moisés Caicedo**, who is recovering from a muscular problem, and **Piero Hincapié**, who has a groin issue. Joel Ordóñez is also reported injured. Enner Valencia is no longer part of the international setup after retiring from national-team football following the World Cup. Willian Pacho becomes one of the senior defensive references, while Nilson Angulo, Pedro Vite, Alan Minda and Keny Arroyo are among the younger attacking and creative options. Experienced goalkeeper Hernán Galíndez was recalled.

**Projected lineups (not confirmed):** no official XI had been released. For South Korea, Kim Min-jae leads the defensive line, Lee Jae-sung and Lee Kang-in are the main midfield and creative names, and Son Heung-min is the reference attacker; Moreno has discussed a new structure, so the shape itself is provisional until his first official team sheet. For Ecuador, Galíndez and Pacho anchor the back line, the midfield has to be rebuilt without Caicedo, and Angulo, Vite, Minda and Arroyo compete for the attacking roles. Hincapié and Caicedo are not expected to feature according to the latest squad information.

### Ecuador lineup update — 23 September

A current Ecuador report projects **Hernán Galíndez; Ángelo Preciado, Félix Torres, Willian Pacho, Pervis Estupiñán; Pedro Vite, Alan Franco, Jordy Alcívar; John Yeboah, Nilson Angulo, Jordy Caicedo**. This is a probable XI, not a confirmed team sheet. Gallardo is also without Piero Hincapié, Joel Ordóñez, Moisés Caicedo, Gonzalo Plata and Kevin Rodríguez; Jordy Caicedo is the leading candidate to fill the centre-forward vacancy left by Enner Valencia's international retirement.

### Injuries, suspensions and availability

Ecuador are expected to be without Caicedo, Hincapié, Ordóñez, Plata and Kevin Rodríguez according to the latest squad information cited below. No injuries or suspensions were reported for South Korea in those sources, but that reflects the reporting available and not a confirmed clean bill of health.

### Match analysis

South Korea's last six matches produced **three wins and three defeats**, with eight goals scored and three conceded. They beat Trinidad and Tobago 5-0, El Salvador 1-0 and the Czech Republic 2-1, while losing 1-0 to Austria, Mexico and South Africa. Five of the six games therefore finished with a South Korea clean sheet or with the opponent scoring exactly once.

Ecuador's corresponding six-match sequence contains **three wins, one draw and two defeats**: victories over Saudi Arabia, Guatemala and Germany, a 0-0 draw with Curaçao, then defeats to Ivory Coast and Mexico. Ecuador scored seven and conceded four in those six. Their defensive baseline remains strong, but the current window starts without two major pieces in Caicedo and Hincapié and under a new coach.

The nations have met twice at senior level according to the KFA's pre-match record, with one win each. South Korea won the most recent meeting **2-0 in Seoul in May 2010**, though that result is too old to carry meaningful weight for the current squads.

The selected market needs South Korea to avoid defeat and the match to reach **two or more total goals**. The first condition is protected by the draw, while a low-scoring 0-0 or 1-0 would still lose the combined selection. That low-scoring outcome is the principal risk to the pick.

**Prediction: South Korea 1X + Over 1.5 goals @ 1.98.**

**Sources:** Korea Football Association (official current squad); Yonhap (30-player selection, fixture and H2H); Ecuador current 26-player call-up reported 20 September; AFP/AS (Gallardo's first squad, Caicedo and Hincapié absences); Sky Sports (recent results).

### Statistical Core Predictions-Sports-Prime

Statistical coverage is partial. The advanced target metrics remain unavailable for a comparable current-cycle South Korea HOME and Ecuador AWAY split, so no xG, xGA, shots, shots on target, possession or corners figures are estimated.

### Market price and value

At published odds of **1.98**, the raw implied probability is **50.5%**, calculated as 1 / 1.98 before bookmaker margin. That market price is distinct from the supplied editorial estimate of **51%** and from the recent-form frequencies described above.

### Conclusion

**Prediction:** South Korea double chance (1X) + Over 1.5 goals  
**Odds:** 1.98`],
  analysisFormat: "markdown",
  editorialStandard: "psp-v1",
  picks: {
    main: "South Korea double chance (1X) + Over 1.5 goals",
    publishedOdds: 1.98,
    oddsProvenance: {
      source: "PSP author-supplied International Friendlies editorial package — 21/09/2026",
      provenance: "author_attested",
      market: "South Korea double chance (1X) + Over 1.5 goals",
    },
  },
  published: true,
  publishedAt: "2026-09-21T12:12:53-03:00",
  updatedAt,
  freshness: {
    editorialUpdatedAt: "2026-09-23T06:50:58-03:00",
    teamNewsUpdatedAt: "2026-09-23T06:50:58-03:00",
    lineupUpdatedAt: "2026-09-23T06:50:58-03:00",
    statisticsUpdatedAt: publishedAt,
  },
  sourceStatus: "partial",
  sources: [
    { name: "Primicias — projected Ecuador lineup against South Korea", url: "https://www.primicias.ec/deportes/seleccion/alineacion-ecuador-marcelo-gallardo-corea-sur-debut-jugadores-amistoso-133189/", description: "Projected Ecuador XI and expanded injury list for Gallardo's debut.", accessedAt: "2026-09-23T06:50:58-03:00" },
    {"name":"Infobae/EFE — Gallardo keeps World Cup core in first Ecuador squad","url":"https://www.infobae.com/espana/agencias/2026/09/20/gallardo-mantiene-base-mundialista-en-su-primera-lista-con-ecuador-con-bajas-por-lesion/","description":"Ecuador squad and injured players.","accessedAt":"2026-09-21T16:17:17-03:00"},
    {
      name: "Korea Football Association — Robert Moreno's 30-player squad",
      url: "https://www.kfa.or.kr/layer_popup/popup_live.php?act=news_tv_detail&check_url=bGF5ZXI%3D&div_code=news&idx=28281&lang=KOR",
      description: "Official South Korea squad and four-match schedule.",
      accessedAt: publishedAt,
    },
    {
      name: "FIFA — Robert Moreno named interim Korea Republic coach",
      url: "https://www.fifa.com/en/tournaments/mens/worldcup/canadamexicousa2026/articles/robert-moreno-named-interim-korea-republic-coach",
      description: "Coaching appointment and first-match fixture context.",
      accessedAt: publishedAt,
    },
    {
      name: "The Korea Times — Moreno's first South Korea squad",
      url: "https://www.koreatimes.co.kr/sports/20260914/dozen-k-leaguers-named-to-morenos-1st-korean-squad",
      description: "Current 30-player selection and domestic-player context.",
      accessedAt: publishedAt,
    },
    {
      name: "AS — Gallardo's first Ecuador squad",
      url: "https://as.com/futbol/internacional/marcelo-gallardo-da-su-primera-lista-con-ecuador-sin-caicedo-ni-hincapie-f202609-n/",
      description: "Ecuador squad and reported Caicedo and Hincapié absences.",
      accessedAt: publishedAt,
    },
  ],
  matchSeo: {
    information: {
      city: "Suwon",
      country: "South Korea",
      timezone: "Asia/Seoul",
      sources: [

        {
          name: "KFA — South Korea international-window squad",
          url: "https://www.kfa.or.kr/layer_popup/popup_live.php?act=news_tv_detail&check_url=bGF5ZXI%3D&div_code=news&idx=28281&lang=KOR",
          accessedAt: publishedAt,
        },
      ],
      updatedAt: publishedAt,
    },
  },
  matchInfo: {
    date: "2026-09-24",
    time: "20:00",
    round: "International Friendly",
    venue: "Suwon World Cup Stadium",
    venueAddress: {
      addressLocality: "Suwon",
      addressCountry: "South Korea",
    },
  },
};
