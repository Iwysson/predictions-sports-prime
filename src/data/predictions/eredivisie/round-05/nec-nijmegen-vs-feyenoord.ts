import type { EditorialPrediction } from "@/types";

export const necNijmegenVsFeyenoord: EditorialPrediction = {
  league: "eredivisie",
  homeTeam: "NEC Nijmegen",
  awayTeam: "Feyenoord",
  slug: "nec-nijmegen-vs-feyenoord",
  analysis: [
    `# NEC Nijmegen vs Feyenoord Prediction, Odds and Betting Tips

**Prediction:** NEC Nijmegen or Draw (1X)
**Odds:** 1.72

### Match Information
**Competition:** Eredivisie
**Date:** 2026-09-05
**Kickoff:** 16:30
**Round:** Matchweek 5
**Venue:** Goffertstadion

### Team News / Availability
Há baixas e dúvidas relevantes nos dois elencos, portanto os nomes devem ser lidos como projeção pré-jogo. No **NEC**, **Philippe Sandler** aparece lesionado, **Sami Ouaissa** enfrenta problema muscular, **Yassin Moslih** está indisponível e **Ahmetcan Kaplan** segue em recuperação de uma lesão no joelho, sendo tratado como situação a acompanhar. Nas três primeiras partidas de **Eredivisie** do recorte atual, foram **oito gols**, média de **2,67 por jogo**, com **xG** acumulado de **7,89**.

### Probable Lineups
**NEC Nijmegen (3-1-4-2)** — Robin Roefs/Crettaz; Calvin Verdonk/Storm, Nemanja Nejašmić, Fonville; Tahaui; Pereira, Monteiro, Dušan Tadić, Noah Naujoks/Bischoff; Tjaronn Chery, Bryan Linssen. **Feyenoord (4-2-3-1)** — Liam Bossin/Ernst; Givairo Read, Tsuyoshi Watanabe, David Møller Wolfe/Màrmol, Jordan Lotomba/Juste; Oussama Targhalline/Vanhoutte, Gjivai Zechiël; Anis Hadj-Moussa, Luciano Valente, Gaoussou Diarra; Ayase Ueda. Há baixas e dúvidas relevantes nos dois elencos, portanto os nomes devem ser lidos como projeção pré-jogo.

### Suspensions / Eligibility
Only absences already supported by the retained pre-match source set are treated as confirmed. No player is removed or inserted merely to complete the template. The official squad list and match sheet override this projection.

### Robust Match Analysis
The analysis is independent of the published pick. The first task is to understand the matchup from the available pre-match evidence; only then is the existing market assessed. The Eredivisie is still in an early 2026/27 sample, so venue-specific averages can move sharply after a single match. That makes it essential to separate HOME from AWAY and to label any fallback explicitly rather than presenting mixed data as if it were a clean venue split.

For NEC Nijmegen, the current HOME profile is **2.50 GF/game**, **1.50 GA/game**, **2.79 xG/game** and **1.73 (overall fallback) xGA/game**. The selected volume context is **16.0 (overall fallback) shots**, **6.0 (overall fallback) SOT**, **55% (overall fallback) possession** and **6.0 (overall fallback) corners for per game**. Because the sample is small, extreme percentages are descriptive observations, not stable probabilities.

For Feyenoord, the AWAY side of the comparison is **3.00 GF/game**, **1.00 GA/game**, **1.92 xG/game** and **1.00 (overall fallback) xGA/game**. The accompanying volume is **17.0 (overall fallback) shots**, **6.0 (overall fallback) SOT**, **60% (overall fallback) possession** and **6.5 (overall fallback) corners for per game**. Where a cell says overall fallback, structural fallback or league baseline, that qualifier is part of the data and prevents the table from pretending that a missing venue feed is a verified venue statistic.

### HOME vs AWAY Data Analysis
NEC's current HOME xG is among the strongest in the league, while Feyenoord's AWAY sample is two wins. Venue-event rows not present in the supplied package are explicitly labelled as overall fallback context. The split is therefore read as a hierarchy of evidence: current venue-specific results and xG first, then team-level current context, then disclosed structural fallback where the supplied package lacks a complete venue feed. No placeholder is converted into zero and no post-match information from this fixture is used.

The defensive-volume comparison also matters. NEC Nijmegen are listed at **12.0 (overall fallback) shots allowed** and **4.0 (overall fallback) SOT allowed** in the selected context, while Feyenoord are at **10.0 (overall fallback)** and **3.5 (overall fallback)** respectively. Corner environment is **10.5** versus **10.5 total corners per match** in the retained context. These figures are useful for describing pressure and game state, but they should not be turned into a deterministic outcome.

### Advanced Data and Expected Game State
The match can change substantially depending on the first goal. If the stronger territorial side scores early, the opponent may be forced into a more aggressive shape, increasing transition space, shots and potentially corners. If the game remains level, both managers can preserve structure longer and reduce the pace. This is especially important for goal markets, where a high pre-match average does not guarantee that the game reaches the threshold.

The xG comparison gives another layer. xG describes chance quality, not finishing certainty. When actual goals run well above xG, regression risk rises; when goals lag behind xG, the attack may have created more than the scoreline suggests. The same applies defensively with xGA. That is why the Core keeps actual GF/GA and xG/xGA side by side rather than allowing one to replace the other.

### Statistical Core Predictions-Sports-Prime
| Metric | NEC Nijmegen — HOME | Feyenoord — AWAY |
| --- | ---: | ---: |
| Matches (N) | 2 | 2 |
| W-D-L | 1-0-1 | 2-0-0 |
| Points/game | 1.50 | 3.00 |
| GF/game | 2.50 | 3.00 |
| GA/game | 1.50 | 1.00 |
| xG/game | 2.79 | 1.92 |
| xGA/game | 1.73 (overall fallback) | 1.00 (overall fallback) |
| Shots/game | 16.0 (overall fallback) | 17.0 (overall fallback) |
| SOT/game | 6.0 (overall fallback) | 6.0 (overall fallback) |
| Shots allowed/game | 12.0 (overall fallback) | 10.0 (overall fallback) |
| SOT allowed/game | 4.0 (overall fallback) | 3.5 (overall fallback) |
| Possession | 55% (overall fallback) | 60% (overall fallback) |
| Corners for/game | 6.0 (overall fallback) | 6.5 (overall fallback) |
| Corners against/game | 4.5 (overall fallback) | 4.0 (overall fallback) |
| Total corners/game | 10.5 | 10.5 |
| First to score | 50% (overall fallback) | 100% (overall fallback) |
| First to concede | 50% (overall fallback) | 0% (overall fallback) |
| Scored in 1st half | 50% (overall fallback) | 100% (overall fallback) |
| Conceded in 1st half | 50% (overall fallback) | 50% (overall fallback) |
| BTTS | 50% | 50% |
| Clean sheets | 0% | 50% |
| Failed to score | 0% | 0% |

### Conflict Detector
The main conflict is sample size and source granularity. Several current venue splits contain only one or two league matches. In addition, some shot, possession, corner or event-rate rows use a clearly labelled overall or structural fallback because the supplied editorial package does not contain a full venue-level feed for both teams. Those fallbacks are context, not invented HOME/AWAY observations. A second conflict is game-state dependence: early red cards, an early goal or an unusually efficient finishing day can overwhelm the pre-match averages.

### Odds and Implied Probability
At decimal odds **1.72**, the raw implied probability is **58.1%** (1 / 1.72 = 58.1%), before bookmaker margin. This is a price-derived threshold, not a proprietary model probability.

### Value Assessment
The published price is retained exactly as supplied. The evidence is strong enough to evaluate whether the market is coherent with the matchup, but the small early-season sample does not justify inventing fair odds or a false precision edge. Value therefore remains conditional on the HOME/AWAY evidence, the disclosed fallbacks and the specific failure routes identified in the Conflict Detector.

### Conclusion
The final position preserves the original published market and price. The analysis supports that decision only to the extent that the current venue split, underlying numbers, lineup context and expected game state point in the same direction. The most important risk is overconfidence in a tiny sample, so the recommendation should be read together with the source qualifiers rather than as a certainty.

**Prediction:** NEC Nijmegen or Draw (1X)
**Odds:** 1.72
`
  ],
  analysisFormat: "markdown",
  editorialStandard: "psp-v1",
  seoTitle: "NEC Nijmegen vs Feyenoord Prediction, Odds & Match Analysis — Eredivisie 2026/27",
  picks: {
    main: "NEC Nijmegen or Draw (1X)",
    publishedOdds: 1.72,
    oddsProvenance: { source: "Supplied 15-analysis editorial sequence package", provenance: "author_attested", market: "NEC Nijmegen or Draw (1X)" },
  },
  published: true,
  publishedAt: "2026-08-30T12:00:00-03:00",
  sourceStatus: "verified",
  sources: [{ name: "OpenFootball — Eredivisie 2026/27 fixtures", url: "https://raw.githubusercontent.com/openfootball/europe/refs/heads/master/netherlands/2026-27_nl1.txt", description: "Season fixture data supporting the competition, round, date, home team and away team used for publication.", accessedAt: "2026-08-30T12:00:00-03:00" }],
  matchInfo: { date: "2026-09-05", time: "16:30", round: "Matchweek 5", venue: "Goffertstadion" },
};
