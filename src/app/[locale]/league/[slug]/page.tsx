...
export function generateStaticParams() { return seoLocaleSlugs.flatMap((locale) => leagues.map((league) => ({ locale, slug: league.slug }))); }
...
export default async function LocalizedLeague({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <>
    ...
    <section className="section"><div className="container"><h2>{copy.matchAnalysis}</h2>{latestMatches.map((match) => {
      const localized = isInternationalMatchExpansionEligible(match) && ["pt-br", "es", "fr", "de", "it"].includes(locale);
